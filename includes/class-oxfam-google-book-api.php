...
class OxfamGoogleBookApi {
	...
	public function search_book( $keyword ) {
		...
		$api_key = $options['oxfam_google_book_api_key'];
	...
	private function is_valid_info_from_google_books_api() {
		if( isset( $this->google_book_api_results['items'] ) &&
		    sizeof( $this->google_book_api_results['items']) > 0 ) {

			$i = 0;

			while( $i < sizeof( $this->google_book_api_results['items'] )
